...
class Keycloak extends AbstractProvider
{
     ...
     * @var string
     */
    public $authServerUrl = "http://192.168.92.132:8080/auth";
    ...
    public function hasRole($specificRole,$token) {
        $decoded = json_decode(base64_decode(str_replace('_', '/', str_replace('-','+',explode('.', $token)[1]))));
        $array = (array) $decoded;
        $rolesarray = (array) $array['realm_access'];
        foreach ($rolesarray['roles'] as $role) {
          if($specificRole==$role) return true;
        }
        return false;
    }
}
